...
function country {
    ...
    if [ -n "$(env | grep 'CTY_FORMAT=DAT')" ]
    then
        #Escapes the square brackets so sed does not get confused
        resCountry=$(echo "${resCountry}" | sed -r "s%([][])%\\\\\1%g")
        resCountry=$(cat "${file}" | tac | sed -n "\%${resCountry}%,/:/ p" | grep ":")
        delimiter=":"
        col=1
    else
        ...
        if [ ! -n "$(echo ${resCountry} | grep "=${1}")" ] && [ -n "$(echo "${1}" | fgrep '\')"]
        then
...
function zones {
    ...
    then
        ...
        radioEntry=$(echo "${radioEntry}" | sed -r "s%([][])%\\\\\1%g")
        ...
        radioEntry="$(cat ${file} | tac | sed -n "\%${radioEntry}%,\%:% p" | grep ":")"
...
function distance {
    read point1_lat point1_long < <(search ${1} | cut -d ',' -f 7,8 | tr ',' ' ')
    read point2_lat point2_long < <(search ${2} | cut -d ',' -f 7,8 | tr ',' ' ')
