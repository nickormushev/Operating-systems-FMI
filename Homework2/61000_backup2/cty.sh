...
function country {
    ...
    #Check to see if it is an exact match by seing if there is an = before the result
    #If that is not the case I want to see if 
    if [ ! -n "$(echo ${res} | grep "=${1}")" ] && [ -n "$(echo "${1}" | fgrep '\')"]
    then
        ...
        echo "$(echo "${res}" | cut -d ',' -f 2) (${foreignCountry})"
        return 0
    fi

    echo "$(echo "${res}" | cut -d ',' -f 2)"
}

function zones {

	arg=$(echo "${1}" | tr '/' 'r')
    radioEntry=$(search ${1} | tr '/' 'r')
